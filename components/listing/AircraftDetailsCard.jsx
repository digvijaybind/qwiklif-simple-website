...
const AircraftDetailsCard = ({
  ...
  console.log('totalTravelDuration', totalTravelDuration);
  ...
  const getTravelDuration = () => {
    ...
    if (segments?.length > 1) {
      ...
      const arrivalTimeZoneOffset = arrivalTime.getTimezoneOffset() / 60;
      const departureTimeZoneOffset = departureTime.getTimezoneOffset() / 60;
      const arrivalTimeGMT = new Date(
        arrivalTime.getTime() + arrivalTimeZoneOffset * 60 * 60 * 1000
      );
      const departureTimeGMT = new Date(
        departureTime.getTime() + departureTimeZoneOffset * 60 * 60 * 1000
      );

      travelDuration += Math.abs(
        new Date(arrivalTimeGMT) - new Date(departureTimeGMT)
      );
      console.log('travelDuration line 113', travelDuration);
    } else {
      // let departureTime = segments[0]?.departure?.at;
